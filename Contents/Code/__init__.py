...
def ProduceRss(title, show_type):
  ...
  for show in shows:
    if show['type'] == show_type:
      ...
      try:
        ...
        oc.add(DirectoryObject(key=Callback(tools.URLError, url=url, show_type=show_type), title="Invalid or Incompatible URL", thumb=R('no-feed.png'), summary="The URL was either entered incorrectly or is incompatible with this channel."))
        continue
  ...
  oc.add(InputDirectoryObject(key=Callback(tools.AddShow, show_type=show_type), title="Add A RSS Feed", thumb=R('feed-add.png'), summary="Click here to add a new RSS Feed", prompt="Enter the full URL (including http://) for the RSS Feed you would like to add"))
